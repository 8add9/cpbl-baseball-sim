...
import { Archetype, BaserunningStrategy, BattingApproach, CareerSkillName, CareerView, createCareer, listCareers, mutateCareer, planCareerWeek, resolveCareerPA, WeeklyAction } from '../api'
...
const PHASES: Record<string, string> = { week_planning: '安排本週', day_ready: '球季進行中', player_pa: '輪到你打擊', post_game: '賽後摘要', season_review: '球季總結', awards: '年度獎項', contract: '合約談判', offseason_training: '休季訓練', ready_next_season: '準備下一季', retired: '已退休' }
const APPROACHES: Array<[BattingApproach, string, string]> = [['normal', '正常揮擊', '不調整能力'], ['aggressive', '積極出棒', '較少保送、提高攻擊性'], ['patient', '耐心選球', '提高選球、略降攻擊'], ['power_swing', '強力揮擊', '提高長打、增加揮空風險'], ['contact', '確實擊球', '降低三振、犧牲長打'], ['situational', '情境打擊', '推進優先、壓低長打']]
...
function Stats({ title, stats }: { title: string; stats: CareerView['season_stats'] }) {
  return <section className="career-card stats-card"><h2>{title}</h2><div className="counting-grid">
    {[['G', stats.games], ['PA', stats.pa], ['R', stats.runs], ['RBI', stats.rbi], ['H', stats.hits], ['HR', stats.home_runs], ['BB', stats.walks], ['SO', stats.strikeouts], ['SB', stats.stolen_bases], ['CS', stats.caught_stealing]].map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}
...
export function CareerMode({ onBack }: { onBack: () => void }) {
  ...
  const [baserunning, setBaserunning] = useState<BaserunningStrategy>('balanced')
  ...
  async function chooseApproach(approach: BattingApproach) { if (career) await command(() => resolveCareerPA(career, approach, baserunning)) }
  ...
  const today = career.calendar_days[career.weekday - 1]
  return <main className="career-shell"><header className="career-topbar"><button onClick={onBack}>← 經理模式</button><strong><span>CPBL</span> 生涯模式</strong><div className="save-state">● 已自動儲存 · rev {career.revision}</div></header>
    ...
    <div className="career-layout">
      <section className="career-card development-card"><div className="development-heading"><div><p className="eyebrow">PLAYER DEVELOPMENT</p><h2>能力與訓練 XP</h2></div><strong>AP {career.action_points_remaining}</strong></div>{SKILLS.map(([key, label]) => { const item = career.skills[key]; return <div className="career-skill" key={key}><div><span>{label}</span>{key === 'speed_proxy' ? <small>影響盜壘成功率與多進一個壘包</small> : null}</div><strong>{item.rating_display}</strong><div className="career-meter"><i style={{ width: `${Math.max(0, Math.min(100, (item.rating_raw - 30) / 80 * 100))}%` }} /></div><span className="score-copy">Score {item.score.toFixed(2)} · XP {item.xp.toFixed(1)}</span></div>})}</section>
      <section className="career-card season-progress"><div><p className="eyebrow">WEEKLY CALENDAR</p><h2>第 {career.week} 週</h2></div><strong>{career.games_played} / 120 場</strong><div className="career-week-grid">{career.calendar_days.map(day => <label className={day.is_game_day ? 'game-day' : ''} key={day.weekday}><strong>週{DAYS[day.weekday - 1]}</strong>{day.is_game_day ? <span>{day.is_home ? '主場' : '客場'} vs {day.opponent_id}</span> : career.phase === 'week_planning' ? <select value={plans[day.weekday] ?? ''} onChange={event => setPlans(current => ({ ...current, [day.weekday]: event.target.value as WeeklyAction | '' }))}><option value="">休息</option>{ACTIONS.map(([value, label, cost]) => <option value={value} key={value}>{label} ({cost} AP)</option>)}</select> : <span>{day.planned_action ?? '休息'}</span>}</label>)}</div>
        ...
        {career.phase === 'day_ready' ? <div className="career-controls">{today?.is_game_day ? <button className="career-primary" disabled={busy} onClick={() => command(() => mutateCareer(career, 'play-game'))}>進入今天比賽</button> : <button className="career-primary" disabled={busy} onClick={() => command(() => mutateCareer(career, 'advance-day'))}>進行今天</button>}<button disabled={busy} onClick={() => command(() => mutateCareer(career, 'simulate-week'))}>模擬本週</button><button disabled={busy} onClick={() => command(() => mutateCareer(career, 'simulate-season'))}>模擬剩餘球季</button></div> : null}
      </section>
      {career.phase === 'player_pa' && career.active_game ? <section className="career-card career-at-bat"><p className="eyebrow">AT BAT · 第 {career.active_game.season_game_number} 場</p><h2>{career.active_game.inning} 局{career.active_game.half === 'top' ? '上' : '下'} · {career.active_game.outs} 出局</h2><div className="career-game-score"><strong>{career.active_game.away_score}</strong><span>－</span><strong>{career.active_game.home_score}</strong></div><div className="career-base-state">壘包：{career.active_game.bases.map((occupied, index) => <span className={occupied ? 'occupied' : ''} key={index}>{index + 1}B</span>)}</div>{career.active_game.last_outcome ? <p>上一打席：{career.active_game.last_outcome}</p> : null}<label>跑壘策略<select value={baserunning} onChange={event => setBaserunning(event.target.value as BaserunningStrategy)}><option value="conservative">保守（少跑、成功率優先）</option><option value="balanced">均衡</option><option value="aggressive">積極（更多盜壘與進壘）</option></select></label><div className="approach-grid">{APPROACHES.map(([value, label, note]) => <button disabled={busy} key={value} onClick={() => chooseApproach(value)}><strong>{label}</strong><small>{note}</small></button>)}</div><button disabled={busy} onClick={() => command(() => mutateCareer(career, 'simulate-game'))}>以正常策略模擬剩餘比賽</button></section> : null}
      {career.phase === 'post_game' ? <section className="career-card career-offseason"><p className="eyebrow">POST GAME</p><h2>比賽結束</h2><p>本季累積：{career.season_stats.hits} 安 · {career.season_stats.home_runs} 轟 · {career.season_stats.stolen_bases} 盜 · {career.season_stats.caught_stealing} 盜壘失敗</p><button className="career-primary" disabled={busy} onClick={() => command(() => mutateCareer(career, 'acknowledge-game'))}>返回週行程</button></section> : null}
